...
class UIBasic:
    ...
    def rent(self, isbn: int) -> None:
        if isbn == "":
            ...
            self._lib.books[isbn].manage_availability = False
    ...
    def give_back(self, isbn: int) -> None:
        if isbn == "":
            ...
            self._lib.books[isbn].manage_availability = True

    def list_available_books(self) -> list:

        if not self._lib.books:
            raise EmptyLibraryError

        available_books = [book for book in self._lib.books.values() if book.manage_availability]
    ...
    def search_by_isbn(self, isbn_num: int) -> Book:
        try:
            ...
            return self._lib.books[isbn_num]
        except KeyError:
    ...
    def search_by_title(self, title_input: str, available_only: bool = True) -> list:
        ...
        books_list_with_title_input = [book for book in self._lib.books.values() if
    ...
    def search_by_author(self, author_input: str, available_only: bool = True) -> list:
        ...
        books_list_with_author_input = [book for book in self._lib.books.values() if
    ...
    def search_by_keyword(self, keyword: str, available_only: bool = True) -> list:
        ...
        books_with_keyword = []
        for book in self._lib.books.values():
